...
record ContextualStructure {ℓₒ ℓₘ} {C : WildCategory ℓₒ ℓₘ} (cwfstr : CwFStructure C)
  ...
  private
    module context-operations where
      dest-context : ∀ {n} (Γ : Con) → len Γ == 1+ n
        → Σ[ Δ ː Con ] Ty Δ × (len Δ == n)
      dest-context = curry (inv-equiv len-∷-equiv)

      match-context : ∀ {n} (Γ : Con) → len Γ == 1+ n → Σ[ Δ ː Con ] Ty Δ
      match-context Γ p = first-two (dest-context Γ p)

      {- proj-context : ∀ {n} {Γ : Con} {p : len Γ == 1+ n} → {!!}
      proj-context Γ .(len Γ) (inl idp) = Γ
      proj-context Γ n (inr n<lenΓ) = {!!} -}
...
module Contextual-contextual-core {ℓₒ ℓₘ} {C : WildCategory ℓₒ ℓₘ} (cwf : CwFStructure C)
  ...
  module _ where
    ...
    listlike O = Lift ⊤
    listlike (1+ n) = Σ[ γ ː listlike n ] Ty (con-of γ)
  ...
  WildCategory.Ob Cᶜ = Σ[ n ː ℕ ] listlike n
