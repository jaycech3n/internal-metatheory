...
record PiStructure {ℓₒ ℓₘ} (ccwf : ContextualCwF ℓₒ ℓₘ) : Type (lsuc (ℓₒ l⊔ ℓₘ)) where

  open ContextualCwF ccwf

  field
    Π′  : ∀ {n} {Γ : Con n} (A : Ty Γ) (B : Ty (Γ ∷ A)) → Ty Γ
    λ′  : ∀ {n} {Γ : Con n} {A : Ty Γ} {B : Ty (Γ ∷ A)}
          → (b : Tm B) → Tm (Π′ A B)
    app : ∀ {n} {Γ : Con n} {A : Ty Γ} {B : Ty (Γ ∷ A)}
          → (f : Tm (Π′ A B)) → Tm B

    βΠ′ : ∀ {n} {Γ : Con n} {A : Ty Γ} {B : Ty (Γ ∷ A)}
          → (b : Tm B) → app (λ′ b) == b
    ηΠ′ : ∀ {n} {Γ : Con n} {A : Ty Γ} {B : Ty (Γ ∷ A)}
          → (f : Tm (Π′ A B)) → λ′ (app f) == f

    Π′[]  : ∀ {m n} {Γ : Con m} {Δ : Con n} {A B} {f : Sub Γ Δ}
            → (Π′ A B) [ f ] == Π′ (A [ f ]) (B [ f ↑ A ])

    λ′[]ₜ : ∀ {m n} {Γ : Con m} {Δ : Con n} {A B} {f : Sub Γ Δ} {b : Tm B}
  ...
  private
    module definitions where
      ...
      →′[] : ∀ {m n} {Γ : Con m} {Δ : Con n} {A B : Ty Δ} {f : Sub Γ Δ}
             → (A →′ B) [ f ] == A [ f ] →′ B [ f ]
      →′[] {A = A} {B} {f} =
        (Π′ A (B [ π A ])) [ f ]
          =⟨ Π′[] ⟩
