...
private
  module equalities where
    ...
    λ=₃ : ∀ {ℓ₁ ℓ₂ ℓ₃ ℓ}
          {A : Type ℓ₁} {B : Type ℓ₂} {C : A → B → Type ℓ₃} {D : Type ℓ}
          {f g : (a : A) (b : B) → C a b → D}
  ...
  module retractions {ℓ₁ ℓ₂} {A : Type ℓ₁} {B : Type ℓ₂} where
    is-retraction : (f : A → B) → Type (ℓ₁ ∪ ℓ₂)
    is-retraction f = ∀ b → hfiber f b

  module reasoning where
    contrapos : ∀ {ℓ₁ ℓ₂} {A : Type ℓ₁} {B : Type ℓ₂} → (A → B) → ¬ B → ¬ A
    contrapos f ¬b a = ¬b (f a)

open equalities public
open equivalences public
open retractions public
