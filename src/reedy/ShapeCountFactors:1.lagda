...
module reedy.ShapeCountFactors:1 {ℓₘ} (I : SimpleSemicategory ℓₘ) where
  ...
  count-factors-all-O-hom-size-O all-O = ¬O<-=O (hom-size j h) bot
    where module _ (O<homjh : O < hom-size j h) where
      ...
      p : count-factors i h (1+ t₀) s₀ f ==
          1+ (count-factors i h t₀ (prev-is-shape s₀) f)
...
rstr-≤ₛ-decr :
  (i h t : ℕ) (s : is-shape i h t) {j : ℕ} (f : hom i j)
  → let cf = count-factors i h t s f in
    (cfs : is-shape j h cf)
  → [ j , h , cf , cfs ]≤[ i , h , t , s ]
rstr-≤ₛ-decr i h t s f cfs = inr (on-𝑖 (hom-inverse _ _ f))
