...
M⃗[ i , O ,1+ t ] s (acc _ rec) {j} f d@(inl yes) rs (acc _ rrec) =
  let
    ...
    -}
    p = ap _[ π (A₀ [ _ ])] ([= ! (comm₀ i t ps pac f prs prac) ] ∙ [◦]) ∙ ![◦]
  in
...
comm₀-aux i t s (acc _ ac) f (inl yes) rfs (acc _ rrec) = assˢᵘᵇ ∙ {!!}
comm₀-aux i t s (acc _ ac) f (inr no) rfs (acc _ rrec) = {!!}

comm₀ i O s (acc _ ac) f rfs (acc _ rrec) = idr (πₜₑₗ •)
comm₀ i (1+ t) s (acc _ ac) f rfs (acc _ rrec) =
  comm₀-aux i t s (acc _ ac) f (discrim i O t (<-from-is-shape s) f) rfs (acc _ rrec)
