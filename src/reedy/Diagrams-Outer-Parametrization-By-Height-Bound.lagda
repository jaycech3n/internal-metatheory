...
M⃗[_] :
  ...
  → let cf = count-factors i h t s f in
    (cfs : is-shape j h cf)
  → Sub (𝔻[ b ] ++ₜₑₗ Mᵒ[ b ] i h t s w) (𝔻[ h ] ++ₜₑₗ Mᵒ[ h ] j h cf cfs lteE)

𝔻[ O ] = ◆ ∷ U
𝔻[ 1+ b ] = 𝔻[ b ] ∷ Πₜₑₗ (Mᵒ[ b ] (1+ b) b (hom-size (1+ b) b) (total-is-shape-1+ b) lteE) U
...
Mᵒ[ 1+ b ] i .(1+ b) O s (inl idp) = wkₜₑₗ $ Mᵒ[ b ] i b (hom-size i b) (full-is-shape i b) lteE
Mᵒ[ 1+ b ] i .(1+ b) (1+ t) s (inl idp) =
  Mᵒ[ 1+ b ] i (1+ b) t prev lteE
  ‣ A [ idd {!!} ◦ˢᵘᵇ {!M⃗[ 1+ b ] i (1+ b) t prev lteE [t] {!!}!} ]
  where
  prev = prev-is-shape s

  A = generic[ _ ; Mᵒ[ b ] (1+ b) b (hom-size (1+ b) b) (total-is-shape-1+ b) lteE ]type

  u = <-from-is-shape s
  [t] = #[ t ] i (1+ b) u

-- Mᵒ[ 1+ b ] i O t s w = wkₜₑₗ $ Mᵒ[ b ] i O t s (O≤ _)
-- Mᵒ[ 1+ b ] i (1+ h) O s w = Mᵒ[ 1+ b ] i h (hom-size i h) (full-is-shape i h) (S≤-≤ w)
-- Mᵒ[ 1+ b ] i (1+ h) (1+ t) s (inr w) = wkₜₑₗ $ Mᵒ[ b ] i (1+ h) (1+ t) s (<S-≤ w)
-- Mᵒ[ 1+ b ] O (1+ .b) (1+ t) s (inl idp) = {!!}
-- Mᵒ[ 1+ b ] (1+ i) (1+ .b) (1+ t) s (inl idp) =
--   Mᵒ[ 1+ b ] (1+ i) (1+ b) t prev lteE
--   ‣ A[1+b] [ idd {!!} ◦ˢᵘᵇ M⃗[ 1+ b ] (1+ i) (1+ b) t prev lteE [t] cfps ]
--   -- Write this induction up! ↑ This is the problem.
--   where
--   prev = prev-is-shape s

--   Mᵒtot[1+b] = Mᵒ[ b ] (1+ b) b (hom-size (1+ b) b) (total-is-shape-1+ b) lteE
--   A[1+b] = generic[ 𝔻[ b ] ; Mᵒtot[1+b] ]type

--   u = <-from-is-shape s
--   [t] = #[ t ] (1+ i) (1+ b) u
--   cfps = count-factors-is-shape (1+ i) (1+ b) t prev [t]
...
M⃗[ 1+ b ] i h t s (inr w) {j} f cfs =
  let
    ...
    Mᵒ[b]tot[1+b] = Mᵒ[ b ] (1+ b) b (hom-size (1+ b) b) (total-is-shape-1+ b) lteE
